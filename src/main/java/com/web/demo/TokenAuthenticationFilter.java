/*
 * TODO #0819 : 토큰 필터 구현 
 * - 종류 (아래 둘 중 하나는 상속받아서 구현후 등록하여 사용)
 *  - OncePerRequestFilter : 동일한 요청에 대해서 한 번만 필터링 하는 용도
...
//		System.out.println("url : " + url);
		...
		if(tokenProvider.isValidToken(accessToken)) {	// 유효함 => 정상 처리 => 인증정보 세팅하여 해당 페이지로 이동
			...
			// 원하는 페이지로 이동
			filterChain.doFilter(request, response);
		}else {	// 유효하지 않다 => 리프레쉬 토큰 활용하여 재발급 => 이후 진행
			System.out.println("유효하지 않은 토큰");
			// 목표 : 엑세스 토큰 재발행
			// 리프레쉬 토큰 검증 진행(토큰 자체는 문제없는지, 디비에 저장되어 있는지)
			if(! tokenProvider.isValidToken(refreshToken)) {
				System.out.println("리프레쉬 토큰 자체가 문제가 존재함");
				response.sendRedirect("/user/signup");
				return;
			}
			// 리프레쉬 오류나면 => 회원가입으로 유도
			if(! tokenProvider.isValidRefreshToken(refreshToken)) {
				System.out.println("리프레쉬 토큰이 디비에 등록되어 있지 않다");
				response.sendRedirect("/user/signup");
				return;
			}
